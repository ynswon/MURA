...
for img, label in samples:
    ...
    study_dir = join(proc_data_dir, enc.study_type)
    study_dir2 = join(study_dir, enc.normal)

    if not os.path.exists(cat_dir):
        os.mkdir(cat_dir)
    if not os.path.exists(study_dir):
        os.mkdir(study_dir)
    if not os.path.exists(study_dir2):
        os.mkdir(study_dir2)

    shutil.copy2(enc.img_filename, join(cat_dir, enc.flat_file_name()))
#    shutil.copy2(enc.img_filename, join(study_dir2, enc.flat_file_name()))
